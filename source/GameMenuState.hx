...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		story = new FlxSprite(0,0).loadGraphic(Paths.image('mainmenu/garry/menu_story_mode'));
		story.setGraphicSize(FlxG.width);
		story.antialiasing = true;
		story.x = 40;
		story.y = 390;
		story.scale.set(0.5, 0.5);
		story.updateHitbox();
		story.scrollFactor.set();
		add(story);
		
		iwanttobefree = new FlxSprite(0,0).loadGraphic(Paths.image('mainmenu/garry/menu_freeplay'));
		iwanttobefree.setGraphicSize(FlxG.width);
		iwanttobefree.antialiasing = true;
		iwanttobefree.x = 40;
		iwanttobefree.y = 480;
		iwanttobefree.scale.set(0.5, 0.5);
